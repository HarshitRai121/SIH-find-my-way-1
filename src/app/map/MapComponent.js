...
import { facilityData } from '../../utils/mapData';
...
function DefaultPopup({ position }) {
  ...
  useEffect(() => {
    if (markerRef.current) {
      ...
      map.setView(position, map.getZoom()); // Ensure the map view is centered
    }
  }, [map, position]);
...
export default function MapComponent() {
  ...
  return (
    <MapContainer center={center} zoom={18} style={{ height: '80vh', width: '100%' }}>
      <TileLayer
